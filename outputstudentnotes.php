...
<?php
  ...
  function describeNote($note) {
    ...
    switch ($nt) {
      ...
      case 1:  return describeAbsenceType($note["absence_type"])." {$note['note_text']}";
...
<div class="resulttable">
    <table class='table table-bordered table-condensed table-striped'>
      <thead><tr class=info><th>Actions</th><th>Note type</th><th>Date Range</th><th>Description</th><th>Created</th><th>Last modified</th></tr></thead>
      <tbody>
  ...
  <?php
    $notes=$PDOX->allRowsDie("SELECT student_note_id,creator_id,student_note.created_at,student_note.updated_at,period_start,period_end,extra_time,note_type,absence_type,note_text,private_text,lti_user.displayname as creatorname
      ...
      student_id={$SID} order by created_at");
    ...
    for ($i=0;$i<sizeof($notes);$i++) {
      ...
      $moddate=date('d/m/Y', strtotime($notes[$i]['updated_at']));
       ...
       echo("<tr><td>{$but}</td><td>".describeNoteType($notes[$i]["note_type"])."</td><td>".describeNotePeriod($notes[$i])."</td><td>".describeNote($notes[$i])."</td><td>{$notes[$i]['creatorname']}<br/>{$crdate}</td><td>{$notes[$i]['creatorname']}<br/>{$moddate}</td></tr>");
    }
   ?>
 </tbody></table>
 </div>
